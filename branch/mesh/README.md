...
| **C: Qwen3-8B** (4096-d, 后补) | **0.8652 ± 0.0058** | **+0.0058 (0.049**)** | **0.8675 ± 0.0053** | **+0.0086 (0.019**)** |
...
> **2026-06-02 补测 Qwen3**：初版只测 4 个生物医学 encoder。后补 Qwen3-Embedding-8B (4096-d, last-token pool) 编同样的 MeSH term（`encode_mesh_qwen3.py`）。**Qwen3 是 5 个里最强的**：Phase2 full +0.0058 (p=0.049)、子集 +0.0086 (p=0.019)，略胜 MedCPT。Phase3 跟其它一样平 (full −0.0006, p=0.91)。→ **推翻了下面第 3 条"专用打败通用"的猜测**：通用强 embedding 在 MeSH 受控术语上不输甚至优于领域专用 encoder。

**结论**：
1. **Qwen3-8B 与 MedCPT MeSH 在 Phase2 都给出真实但小幅的正增益**（Qwen3 full +0.0058 p=0.049 / 子集 +0.0086 p=0.019；MedCPT full +0.0046 p=0.039 / 子集 +0.0073 p=0.015），n=10 配对检验显著。**这是 SMILES + MeSH 两条 branch 里唯一稳健显著的正信号。**
2. **Phase3 无效**（全部 p>0.6）。增益不跨 phase 稳定 —— Phase3 样本少、B' 已高(0.89)，余量小。
3. **encoder 排序：Qwen3-8B ≳ MedCPT ≫ BioLORD/PubMedBERT/SapBERT**。对 SAE 有用的是"疾病-药物-不良事件"的**共现/文献语义**——通用大模型 Qwen3 和文献对比的 MedCPT 都抓得住；实体规范化 encoder（SapBERT UMLS 同义词 / BioLORD 概念定义）不对口。**通用强 embedding 在 MeSH 上不输领域专用**（原以为专用会赢，被 Qwen3 推翻）。
...
| **MeSH（疾病+药物术语，5 encoder）** | **Qwen3 +0.0058 / MedCPT +0.0046 full (p<0.05)** | 平 | **Phase2 小幅稳健正** |
